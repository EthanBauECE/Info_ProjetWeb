...
try {
    ...
    $_SESSION['profile_error'] = "Erreur de connexion à la base de données.";
...
function safe_html($value) {
    // Ensures htmlspecialchars always receives a string, even if $value is null.
...
$type = $user_data['TypeCompte'];

// --- Handle Form Submission ---
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Collect data - use user_data as fallback if POST is incomplete or for unchanged fields
    $nom = trim($_POST['nom'] ?? $user_data['Nom']);
    $prenom = trim($_POST['prenom'] ?? $user_data['Prenom']);
    $email = trim($_POST['email'] ?? $user_data['Email']);

    // Address fields - new granular fields
    ...
    $adresse_id = $user_data['AdresseID']; // Get existing address ID
    ...
    if ($type === 'client') {
        ...
        // Description and Specialite can be NULL in DB, so empty string is fine.
        $description = trim($_POST['description'] ?? $user_data['Description']);
        $specialite = trim($_POST['specialite'] ?? $user_data['Specialite']);
    }

    // Basic Validation (aligning with NOT NULL DB constraints)
    ...
    // Address validation (required by DB schema)
    ...
    // infos_complementaires is NOT NULL but can be an empty string, so no empty check needed here.

    // Type-specific validation (required by DB schema)
    if ($type === 'client' || $type === 'personnel') {
        // Regex for phone number: starts with 06 or 07, followed by 8 digits.
        if (empty($telephone) || !preg_match('/^0[67]\d{8}$/', $telephone)) {
            $form_errors['telephone'] = "Numéro de téléphone invalide (Ex: 0612345678).";
        }
    }
    if ($type === 'client') {
        // Carte Vitale: 13 to 15 digits
    ...
    if (empty($form_errors)) {
        ...
        try {
            ...
            $stmt_update_user->execute([$nom, $prenom, $email, $user_id]);
            ...
            if ($adresse_id) {
                ...
                $stmt_update_address->execute([$adresse_ligne, $ville, $code_postal, $infos_complementaires, $adresse_id]);
            } else {
                // If user had no address associated, create one and link it.
                $sql_insert_address = "INSERT INTO adresse (Adresse, Ville, CodePostal, InfosComplementaires) VALUES (?, ?, ?, ?)";
                $stmt_insert_address = $pdo->prepare($sql_insert_address);
                $stmt_insert_address->execute([$adresse_ligne, $ville, $code_postal, $infos_complementaires]);
                $new_adresse_id = $pdo->lastInsertId();

                // Link new address ID to user_client or user_personnel
                if ($type === 'client') {
                ...
                $stmt_link_address = $pdo->prepare($sql_link_address);
                $stmt_link_address->execute([$new_adresse_id, $user_id]);
            }

            // Update type-specific table
            if ($type === 'client') {
                $sql_update_client = "UPDATE utilisateurs_client SET Telephone = ?, CarteVitale = ? WHERE ID = ?";
                $stmt_update_client = $pdo->prepare($sql_update_client);
                $stmt_update_client->execute([$telephone, $carte_vitale, $user_id]);
            } elseif ($type === 'personnel') {
                $sql_update_personnel = "UPDATE utilisateurs_personnel SET Telephone = ?, Description = ?, Type = ? WHERE ID = ?";
                $stmt_update_personnel = $pdo->prepare($sql_update_personnel);
                $stmt_update_personnel->execute([$telephone, $description, $specialite, $user_id]);
            }
            ...
            // Merge submitted data to user_data for re-display in case of DB error
            $user_data = array_merge($user_data, $_POST); 
            // Also need to handle specific address fields for re-population
            $user_data['AdresseLigne'] = $_POST['adresse_ligne'];
            $user_data['Ville'] = $_POST['ville'];
            $user_data['CodePostal'] = $_POST['code_postal'];
            $user_data['InfosComplementaires'] = $_POST['infos_complementaires'];
        }
    } else {
        // If there are validation errors, re-populate form with POST data
        $user_data = array_merge($user_data, $_POST);
        // Also need to handle specific address fields for re-population
        $user_data['AdresseLigne'] = $_POST['adresse_ligne'];
        $user_data['Ville'] = $_POST['ville'];
        ...
        <form action="modifier_profil.php" method="POST" class="profile-edit-form">
            <section class="profile-section">
                ...
                <div class="form-group">
                    ...
                    <?php if (isset($form_errors['nom'])) echo "<p class='error-message'>".$form_errors['nom']."</p>"; ?>
                </div>
                <div class="form-group">
                    <label for="prenom">Prénom :</label>
                    <input type="text" id="prenom" name="prenom" value="<?= safe_html($user_data['Prenom']) ?>" required>
                    <?php if (isset($form_errors['prenom'])) echo "<p class='error-message'>".$form_errors['prenom']."</p>"; ?>
                </div>
                <div class="form-group">
                    <label for="email">Email :</label>
                    <input type="email" id="email" name="email" value="<?= safe_html($user_data['Email']) ?>" required>
                    <?php if (isset($form_errors['email'])) echo "<p class='error-message'>".$form_errors['email']."</p>"; ?>
                </div>
            </section>
            ...
            <section class="profile-section">
                ...
                <div class="form-group">
                    ...
                    <?php if (isset($form_errors['adresse_ligne'])) echo "<p class='error-message'>".$form_errors['adresse_ligne']."</p>"; ?>
                </div>
                <div class="form-group">
                    <label for="ville">Ville :</label>
                    <input type="text" id="ville" name="ville" value="<?= safe_html($user_data['Ville']) ?>" required>
                    <?php if (isset($form_errors['ville'])) echo "<p class='error-message'>".$form_errors['ville']."</p>"; ?>
                </div>
                <div class="form-group">
                    ...
                    <?php if (isset($form_errors['code_postal'])) echo "<p class='error-message'>".$form_errors['code_postal']."</p>"; ?>
                </div>
                <div class="form-group">
            ...
            <?php if ($type === 'client'): ?>
                <section class="profile-section">
                    ...
                    <div class="form-group">
                        ...
                        <?php if (isset($form_errors['telephone'])) echo "<p class='error-message'>".$form_errors['telephone']."</p>"; ?>
                    </div>
                    <div class="form-group">
                        ...
                        <?php if (isset($form_errors['carte_vitale'])) echo "<p class='error-message'>".$form_errors['carte_vitale']."</p>"; ?>
                    </div>
                </section>
            <?php elseif ($type === 'personnel'): ?>
                <section class="profile-section">
                    ...
                    <!-- Photo and Video modification would require file upload handling -->
                    <!-- For simplicity, these fields are not editable via this form -->
